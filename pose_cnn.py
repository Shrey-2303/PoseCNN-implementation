...
class SegmentationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 64):
        ...
        self.conv1_feature1 = nn.Conv2d(512, hidden_layer_dim, kernel_size=1)
        self.relu1_feature1 = nn.ReLU(inplace=True)

        self.conv1_feature2 = nn.Conv2d(512, hidden_layer_dim, kernel_size=1)
        self.relu1_feature2 = nn.ReLU(inplace=True)

        self.final_conv = nn.Conv2d(hidden_layer_dim, num_classes + 1, kernel_size=1)
    ...
    def forward(self, feature1, feature2):
        ...
        feature1 = self.relu1_feature1(self.conv1_feature1(feature1))
        feature2 = self.relu1_feature2(self.conv1_feature2(feature2))
        # feature1_upsampled = F.interpolate(feature1, size=feature2.shape[2:], mode='bilinear', align_corners=False)
        # combined_features = feature1_upsampled + feature2
        
        self.input_size = (480,640)
        feature2_upsampled = F.interpolate(feature2, size=feature1.shape[2:], mode='bilinear', align_corners=False)
        combined_features = feature1 + feature2_upsampled
        upsampled_features = F.interpolate(combined_features, size=self.input_size, mode='bilinear', align_corners=False)
        probability_map = self.final_relu(self.final_conv(upsampled_features))

        #probability_map = self.final_conv(upsampled_features)
        probability = F.softmax(probability_map, dim=1)
        ...
        # feature1_processed = self.relu(self.conv1x1_feature1(feature1))
        # feature2_processed = self.relu(self.conv1x1_feature2(feature2))
        # feature2_upsampled = F.interpolate(feature2_processed, size=feature1_processed.shape[2:], mode='bilinear', align_corners=False)
        
        # combined_features = feature1_processed + feature2_upsampled
        ...
        return probability_map, segmentation, bbx
...
class TranslationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 128):
        ...
        # Define convolutional layers for the translation estimation
        self.conv1 = nn.Conv2d(512, hidden_layer_dim, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(512, hidden_layer_dim, kernel_size=3, padding=1)

        self.relu1_conv1 = nn.ReLU(inplace=True)
        self.relu2_conv2 = nn.ReLU(inplace=True)



        self.final_conv = nn.Conv2d(hidden_layer_dim, num_classes * 3, kernel_size=1)
    ...
    def forward(self, feature1, feature2):
        ...
        # print("hi")
        # feature1_processed = F.relu(self.conv1(feature1))
        # print(feature1_processed.shape)
        # feature2_upsampled = F.interpolate(feature2, size=feature1_processed.shape[2:], mode='bilinear', align_corners=False)
        # print(feature2_upsampled.shape)
        #feature2_processed = F.relu(self.conv2(feature2_upsampled))
        #combined_features = feature1_processed + feature2_upsampled
        # translation = self.final_conv(combined_features)

        # feature1_processed = F.relu(self.conv1(feature1))
        # feature2_processed = F.relu(self.conv2(feature2))
        # feature2_upsampled = F.interpolate(feature2_processed, size=feature1_processed.shape[2:], mode='bilinear', align_corners=False)
        # combined_features = feature1_processed + feature2_upsampled
        # upsampled_features = F.interpolate(combined_features, size=self.input_size, mode='bilinear', align_corners=False)
        # translation = self.final_conv(upsampled_features)
        


        feature1 = self.relu1_conv1(self.conv1(feature1))
        feature2 = self.relu2_conv2(self.conv2(feature2))
...
class RotationBranch(nn.Module):
    ...
    def __init__(self, feature_dim = 512, roi_shape = 7, hidden_dim = 4096, num_classes = 10):
        ...
        # self.roi_pool1 = RoIPool
        # self.roi_pool2 = ops.roi_align
        # self.fc1 = nn.Linear(feature_dim * roi_shape * roi_shape, 4096)
        # self.fc2 = nn.Linear(4096, 4096)
        # self.fc3 = nn.Linear(4096, 4 * num_classes)
        self.num_classes = num_classes

        self.roi_pool1 = RoIPool(output_size=(roi_shape, roi_shape), spatial_scale=1.0/16)
        self.roi_pool2 = RoIPool(output_size=(roi_shape, roi_shape), spatial_scale=1.0/16)
        self.fc1 = nn.Linear(feature_dim * roi_shape * roi_shape *2 , hidden_dim)  # *2 for the two pooled features
    ...
    def forward(self, feature1, feature2, bbx):
        ...
        # combined_features = feature1
        # quaternions = torch.zeros((bbx.size(0), 4 * self.num_classes), device=bbx.device)
        
        # for i, (batch_id, x1, y1, x2, y2) in enumerate(bbx):
        #     roi_features = combined_features[batch_id, :, y1:y2, x1:x2]
        #     roi_pooled = self.roi_pool(roi_features.unsqueeze(0))
            
        #     roi_flattened = roi_pooled.view(1, -1)
        #     fc_output = F.relu(self.fc1(roi_flattened))
        #     fc_output = F.relu(self.fc2(fc_output))
        #     quaternion = self.fc3(fc_output)  # (1, 4*num_classes)
            
        #     quaternions[i] = quaternion


        # print(bbx.shape)
        pooled_features1 = self.roi_pool1(feature1, bbx)
        pooled_features2 = self.roi_pool2(feature2, bbx)
        
        #print(pooled_features1.shape)
        #print(pooled_features2.shape)
        
        flattened_features1 = pooled_features1.view(pooled_features1.size(0), -1)
        flattened_features2 = pooled_features2.view(pooled_features2.size(0), -1)
        combined_features = torch.cat((flattened_features1, flattened_features2), dim=1)
        
        fc_output = F.relu(self.fc1(combined_features))
        fc_output = F.relu(self.fc2(fc_output))
        quaternion = self.fc3(fc_output).view(-1, self.num_classes * 4)  
...
class PoseCNN(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            #for keys, values in input_dict.items(): print(values.shape)
            feature1, feature2 = self.feature_extraction(input_dict)
            probability, segmentation, bbx = self.segmentation_branch(feature1, feature2)
            gt_labels_indices = torch.argmax(input_dict['label'].float(), dim=1)  # Converts to [B, H, W]
            loss_segmentation = F.cross_entropy(probability, gt_labels_indices)

            loss_dict["loss_segmentation"] = loss_segmentation

            # print(bbx.shape)
            # quaternion_map = self.rotation_branch(feature1, feature2, bbx[:,:5].float())
            ...
            #gt_masks = input_dict['label'].float()  
            #print(segmentation.shape)
            ...
            # print(gt_bbx.shape)
            # print(bbx.shape)
            # selected = IOUselection(bbx, gt_bbx, self.iou_threshold)
            # print(selected.shape)
            # pred_translations = self.estimateTrans(translation_map, bbx, segmentation)
            # gt_translations = self.gtTrans(selected, input_dict)
            translation_map = self.translation_branch(feature1, feature2)
            loss_translation = F.l1_loss(translation_map,input_dict['centermaps'])
            loss_dict["loss_centermap"] = loss_translation

            # Rotation 

            # pred_rotations, labels = self.estimateRotation(quaternion_map, bbx)
            # gt_rotations = self.gtRotation(gt_bbx, input_dict)
            # loss_rotation = loss_Rotation(pred_rotations, gt_rotations, labels, self.models_pcd)
            filter_bbx = IOUselection(bbx, gt_bbx, self.iou_threshold)
            
            if len(filter_bbx) != 0:
                pred_R = self.rotation_branch(feature1, feature2, filter_bbx[:,:-1])    #[N, 40]
                pred_R, labels = self.estimateRotation(pred_R, filter_bbx)              # pred_R = [N, 3, 3]
                gt_R = self.gtRotation(filter_bbx, input_dict)                        #[N, 3, 3]
                loss_rotation = loss_Rotation(pred_R, gt_R, labels, self.models_pcd)
            ...
            with torch.no_grad():
                ...
                # # centermap = input_dict.get('centermap', None)  # Placeholder: compute or get centermap accordingly
                # # centers, depths = HoughVoting(segmentation, centermap, num_classes=self.segmentation_branch.num_classes)
                # # gt_bbx = self.getGTbbx(input_dict)  
                # # selected_bbx = IOUselection(bbx, gt_bbx, self.iou_threshold)


                # # translation_map = self.translation_branch(feature1, feature2)
                # # quaternion_map = self.rotation_branch(feature1, feature2, selected_bbx)

                # # pred_translations = self.estimateTrans(translation_map, selected_bbx, segmentation)
                # # pred_rotations, _ = self.estimateRotation(quaternion_map, selected_bbx)
                # # output_dict = self.generate_pose(pred_rotations, centers, depths, selected_bbx)

                # output_dict = {
                #     'probability': probability,
                #     'segmentation': segmentation, }
                #     # 'translation': translation_map, 
                # #     'quaternion': quaternion_map,
                # #     'bbx': bbx
                # # }
